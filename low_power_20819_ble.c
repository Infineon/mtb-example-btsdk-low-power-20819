...
#define NOTFICATION_TIME_MS    5000  /* 5 seconds interval for
                                        notification if notifications enabled
                                        by client*/
...
uint8_t low_power_20819_current_state = SLEEP_WITHOUT_BLE;

/* Maintains the connection id of the current connection */
uint8_t low_power_20819_conn_id       = DUMMY;

/* Notification timer handle */
wiced_timer_t  low_power_20819_notification_timer_handle;
...
static wiced_bt_gatt_status_t low_power_20819_gatt_write_handler
                       (wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id);

static wiced_bt_gatt_status_t low_power_20819_gatt_read_handler
                         (wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id);

static wiced_bt_gatt_status_t low_power_20819_gatt_connect_callback
                           (wiced_bt_gatt_connection_status_t *p_conn_status);

static wiced_bt_gatt_status_t low_power_20819_gatt_server_callback
(uint16_t conn_id, wiced_bt_gatt_request_type_t type,
                                         wiced_bt_gatt_request_data_t *p_data);

static wiced_bt_gatt_status_t low_power_20819_gatt_event_handler
...
void low_power_20819_app_init(void)
{
    ...
    if(WICED_BT_SUCCESS != wiced_init_timer(
                           &low_power_20819_notification_timer_handle,
                           &low_power_20819_timeout,
                           DUMMY,
    ...
    if(WICED_BT_GATT_SUCCESS != wiced_bt_gatt_register(
                                low_power_20819_gatt_event_handler))
    {
        WICED_BT_TRACE("GATT callback registration failed\r\n");
    }

    /* Initialize GATT Database */
    if(WICED_BT_GATT_SUCCESS != wiced_bt_gatt_db_init(
                                gatt_database, gatt_database_len))
    {
...
void low_power_20819_set_advertisement_data(void)
{
    ...
    if(WICED_BT_SUCCESS != wiced_bt_ble_set_raw_advertisement_data(
                           num_elem, adv_elem))
    {
...
{
    /* Check if device is connected and notifications
     * are enabled and send notification */
    if((GATT_CLIENT_CONFIG_NOTIFICATION == app_bas_battery_level_client_char_config[0])
                                           && (0 != low_power_20819_conn_id))
    {
       ...
       if(WICED_BT_GATT_SUCCESS != wiced_bt_gatt_send_notification(low_power_20819_conn_id,
...
/*******************************************************************************
* Function Name: wiced_bt_dev_status_t low_power_20819_bt_management_callback(
...
*
*******************************************************************************/
wiced_bt_dev_status_t low_power_20819_bt_management_callback(
                      wiced_bt_management_evt_t event,
                      wiced_bt_management_evt_data_t *p_event_data)
{
...
/*******************************************************************************
* Function Name: wiced_bt_gatt_status_t low_power_20819_get_value(
...
*
*******************************************************************************/
wiced_bt_gatt_status_t low_power_20819_gatt_write_handler(wiced_bt_gatt_write_t
                                                *p_write_req, uint16_t conn_id)
{
    ...
    if(HDLD_BAS_BATTERY_LEVEL_CLIENT_CHAR_CONFIG == p_write_req->handle)
    {
        ...
        {
            if(WICED_BT_SUCCESS != wiced_stop_timer(
                                   &low_power_20819_notification_timer_handle))
            {
...
*
*********************************************************************************/
wiced_bt_gatt_status_t low_power_20819_gatt_read_handler(
                        ...
                        uint16_t conn_id)
{
    ...
    status = low_power_20819_get_value(p_read_req->handle,
                                       conn_id, p_read_req->p_val,
...
*
*******************************************************************************/
wiced_bt_gatt_status_t low_power_20819_gatt_connect_callback(
                       wiced_bt_gatt_connection_status_t *p_conn_status)
{
    ...
    if (NULL != p_conn_status)
    {
        if (p_conn_status->connected)
        {
            ...
            if(TRUE != wiced_bt_l2cap_update_ble_conn_params(
        ...
        else
        {
            ...
            low_power_20819_current_state = HIDOFF;
            // stop notification timer if it was turned on to allow device to go to HID off
            wiced_stop_timer(&low_power_20819_notification_timer_handle);
...
/*******************************************************************************
 * Function Name: wiced_bt_gatt_status_t low_power_20819_gatt_server_callback(
 *                uint16_t conn_id,
...
/*******************************************************************************
 * Function Name: wiced_bt_gatt_status_t low_power_20819_gatt_event_handler(
 ...
 *
 ******************************************************************************/
wiced_bt_gatt_status_t low_power_20819_gatt_event_handler(wiced_bt_gatt_evt_t event,
                                       wiced_bt_gatt_event_data_t *p_event_data)
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATT_CONNECTION_STATUS_EVT:
        status = low_power_20819_gatt_connect_callback(
                 &p_event_data->connection_status);
        break;
    case GATT_ATTRIBUTE_REQUEST_EVT:
        p_attr_req = &p_event_data->attribute_request;
        status = low_power_20819_gatt_server_callback(p_attr_req->conn_id,
 ...
 *******************************************************************************
 * Summary: Callback on button press. It checks the current state of the
 *          system and takes appropriate action
 *
 * Parameters:
